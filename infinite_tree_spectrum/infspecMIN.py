...
def choose_vertex(G):
    ...
    for v in degree1:
        if not v in G:
            continue
        root = G[v][0]
        if root in good_vertices or root in star:
            continue
        d1 = 0
        d2 = 0
        for u in G[root]:
...
def DFS_visit(G, neigh):  #If it finds one star vertex with even distance, it returns True
    global visited
    global status
    visited[neigh] = 2
...
def subSpec(G):
    ...
    if (not is_connected(G)):
        ...
        for grp in subgrps:
            res = res + subSpec(subgraph(G, grp))
        return res
    ...
    root, dg1, dg2 = choose_vertex(G)
    # this block uses the theorem to calc spec recursively
    res = Set([])
    H = G.copy()
    H.delete_nodes_from([root, dg1])
    #print root, dg1
    res = Set([1]) + subSpec(H)
    if (dg2 != -1):
        #infForceZero(G)
        if (checkForces_2ray(G, root, dg2)==False):
            #print G.edges(), root, dg2
            res = add_minim(res)
    return res
...
def HTree(i):
    ...
    edges[50] = [(0, 1), (0, 2), (0, 4), (0, 6), (1, 8), (1, 10), (2, 3), (2, 5), (2, 7), (5, 9), (5, 11)]
    res  [50] = [2, 3]
    G = Graph()
